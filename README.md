# Somfy Roller Blind BLE → Home Assistant

An ESP32 bridge that drives **Somfy Sonesse2 Zigbee** roller-blind motors over their
**Bluetooth Low Energy (BLE)** GATT interface and exposes them to Home Assistant as native
`cover` entities over MQTT — no Zigbee coordinator, TaHoma Pro account, or Somfy remote required.
...
**Phase S — firmware framework scaffold.** The reusable base (WiFi provisioning, local web UI,
OTA, MQTT client) builds and boots; BLE bring-up against real hardware is next. See
...
updater), `Mqtt` (PubSubClient + HA discovery), all backed by an NVS `AppConfig`. See
[decisions/0001-framework-reuse.md](docs/decisions/0001-framework-reuse.md).
...
not affiliated with, endorsed by, or condoned by Somfy.
